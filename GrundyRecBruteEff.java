 ...
 * This version contains only the functional improvements to make the game playable.
...
class GrundyRecBruteEff {
     ...
     * @return true if a winning move exists and has been played, false otherwise
     */
        ...
        if (jeu == null) {
            ...
            if ( !estPossible(jeu) ) {
                ...
                while ( (tas != -1) && ret) {
                    ...
                    // so game state is not losing so we get out of the while loop and return falsse
    ...
    void testEstGagnanteEfficacite() {
        ...
        while (diffT<500000000) { //We try until execution time exceeds 500 milliseconds
            jeu = new ArrayList<>(); //Game board reset
            jeu.add(n); // heap containing n matches
    
            cpt = 0; // Counter reset
            debut = System.nanoTime(); // Start timing
    
            estGagnante(jeu); // call tested method
    
            fin = System.nanoTime(); // End timing
            diffT = (fin - debut); // Calculates execution time
